...
def render_account_status(request: gr.Request):
    ...
    if is_admin:
        ...
        <div class="account-card pro-card">
            ...
            <div class="account-details">Acceso total: 4 Voces, Shadowing, Audiolibros ZIP, Subtítulos .SRT/.VTT y Pack ZIP.</div>
        </div>
        """
    else:
        return f"""
        <div class="account-card guest-card">
            ...
            <div class="account-details">
                ...
                • Podcast: máx. 10 min (2 voces)<br>
                • Shadowing / Libros / Video: Bloqueados (PRO)
            </div>
...
# =========================================================
# MOTOR DE SÍNTESIS ROBUSTO (SIN ERRORES)
# =========================================================
...
# 3. Podcast Studio (Hasta 4 Voces)
async def fn_podcast_studio(script, v1_lbl, v2_lbl, v3_lbl, v4_lbl, speed_lbl, request: gr.Request):
    ...
    if not is_admin and (words > GUEST_MAX_WORDS_PODCAST or chars > GUEST_MAX_CHARS_PODCAST):
        return (
            ...
            f"🎙️ **¿Deseas compilar podcasts extensos y hasta 4 voces?** Actualiza a la versión **PRO** contactando directamente al desarrollador."
        )
    ...
    speed_val = SPEEDS.get(speed_lbl, 0)
    ...
    try:
        for line in lines:
            if ":" in line:
                ...
                if any(k in tag_low for k in ["4", "narrador", "voz en off", "narrator", "locutor 4"]):
                    selected_voice = v4 if is_admin else v1
                elif any(k in tag_low for k in ["3", "especialista", "guest 2", "invitado 2", "locutor 3"]):
                    selected_voice = v3 if is_admin else v1
    ...
    try:
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            for i, sentence in enumerate(sentences, 1):
                ...
                # Estimar duración en segundos de la frase
                duration_sec = max(2.5, len(sentence.split()) * 0.45)
                pause_sec = duration_sec * 1.3  # Pausa generosa para repetir
                
                # Escribir en el master audio
                ...
                # Guardar pista individual dentro del ZIP
        ...
        return master_file, zip_file, f"✅ [{badge}] Sesión de Shadowing generada ({len(sentences)} unidades de práctica + Pack ZIP)."
    except Exception as e:
        return None, None, f"❌ Error: {str(e)}"

# 5. Audiolibros por Capítulos en ZIP (PRO)
    ...
    try:
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            for i, chap in enumerate(chapters, 1):
                ...
                if chunk:
                    master_audio.write(chunk)
                    master_audio.write(generate_mp3_silence(2.5))  # Pausa entre capítulos
        ...
        return master_file, zip_file, f"✅ [{badge}] Audiolibro compilado ({len(chapters)} capítulos empaquetados en ZIP)."
    except Exception as e:
        return None, None, f"❌ Error: {str(e)}"
...
async def fn_video_voiceover(text, voice_label, style_speed, request: gr.Request):
    ...
    is_admin, badge, _ = check_user_access(request)
    if not is_admin:
        return None, None, None, PRO_UPGRADE_MSG
    ...
    try:
        ...
        return mp3_file, srt_file, vtt_file, f"✅ [{badge}] Locución y subtítulos (.SRT + .VTT) generados con éxito."
    except Exception as e:
...
def get_all_media_files(request: gr.Request):
    ...
    if not is_admin:
        return files[:3], None, "ℹ️ *Modo Invitado: Mostrando tus 3 archivos recientes. Actualiza a PRO para historial y descarga en ZIP.*"
    return files, None, "✅ *Historial completo de archivos disponible (Modo Admin).*"
...
def download_all_as_zip(request: gr.Request):
    ...
    files = [f for f in os.listdir(".") if f.endswith(".mp3") or f.endswith(".srt") or f.endswith(".vtt")]
    if not files:
    ...
    return zip_name, f"✅ [{badge}] Pack ZIP generado con {len(files)} archivos listos para descargar."

def clear_all_media_files(request: gr.Request):
    is_admin, _, _ = check_user_access(request)
    if not is_admin:
        return [], None, "⚠️ Solo los administradores PRO pueden purgar el almacenamiento.", []
...
with gr.Blocks(title="Text to Speech Pro Studio", css=custom_css) as demo:
        ...
        with gr.Column(elem_classes=["content-area"]):
            ...
            with gr.Column(visible=True, elem_classes=["card-box"]) as view_editor:
                ...
                with gr.Row(elem_classes=["top-bar-controls"]):
                    ...
                    main_speed = gr.Dropdown(choices=list(SPEEDS.keys()), value="1.0x (Normal / Conversacional)", show_label=False, scale=1)
            ...
            # 2. Podcast Studio (4 Voces)
            with gr.Column(visible=False, elem_classes=["card-box"]) as view_podcast:
                gr.Markdown("### 🎧 Podcast Studio Multi-Voz *(Hasta 4 Participantes)*")
                with gr.Row():
                ...
                with gr.Row():
                    pod_v3 = gr.Dropdown(choices=list(VOICES.keys()), value="🇺🇸 Brian (Documental / Autoridad)", label="Locutor 3 (Invitado 2 - Solo PRO)")
                    pod_v4 = gr.Dropdown(choices=list(VOICES.keys()), value="🇺🇸 Ava (Joven / Dinámica)", label="Locutor 4 (Narrador - Solo PRO)")
    ...
    btn_gen_podcast.click(
        ...
        inputs=[pod_text, pod_v1, pod_v2, pod_v3, pod_v4, pod_spd],
